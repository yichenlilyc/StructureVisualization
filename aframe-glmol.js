...
/***/ (function(module, __webpack_exports__, __webpack_require__) {
    ...
    */
    ...
    var ModelUrl = "http://192.168.1.6:5500/html/modelupload";
    if (DEPLOY) {
        ModelUrl = "https://structureviz.wpi.edu/modelupload";
    }
    // import {ModelUrl} from  "./config.js"
    ...
    var GLmol = (function() {
          ...
          function GLmol(data) {
            ...
            GLmol.prototype.download = function(resolve, reject) {
              ...
              var description = this.data.pdb;
              if (query.substr(0,4)=='lot:') {
                _this.loadMoleculeStr(description);
                _resolve(_this.modelGroup);
              } else if (query.substr(0, 4) == 'lof:') {
                ...
                input.onchange = function(){
                  ...
                  reader.onload = function() {
                    document.getElementById("M1Tab").innerHTML= query.substr(4,4).toUpperCase();
                    _this.loadMoleculeStr(reader.result);
                    _resolve(_this.modelGroup)
                  };
                ...
                } else if (query.substr(0, 4) == 'wpi:') {
                  query = query.substr(4);
                  // if (!query.match(/^[1-9][A-Za-z0-9]{3}$/)) {
                  //   alert("Wrong Model ID");
                  //   return;
                  // }
                  // alert(query);
                  uri = ModelUrl + "/" + query + ".pdb";
                  // alert(uri);
                }
              ...
              for (var i = 0; i < lines.length; i++) {
                ...
                if (recordName == 'ATOM  ' || recordName == 'HETATM') {
                  ...
                  if (elem == '') { // for some incorrect PDB files
                    ...
                    // elem = line.substr(12, 2).replace(/ /g, ""); //---lgz
                  }
                  if (line[0] == 'H')
                    hetflag = true;
                  else
            ...
            GLmol.prototype.drawAtomsAsSphere = function(group, atomlist, defaultRadius, forceDefault, scale) {
              // var sphereGeometry = new THREE.SphereGeometry(1, this.sphereQuality, this.sphereQuality); ...lgz
              // r, seg, ring
            ...
            GLmol.prototype.drawBondAsStickSub = function(group, atom1, atom2, bondR, order) {
              ...
              // var c1 = new TCo(atom1.color),  ...lgz
              //   c2 = new TCo(atom2.color);     ...lgz
              if (order == 1 || order == 3) {
            ...
            GLmol.prototype.drawMainchain = function(asu, mainchainMode, doNotSmoothen) {
              ...
              if (mainchainMode == 'ribbon') {
                ...
                this.drawBondsAsStick(asu, all, this.cylinderRadius / 2.0, this.cylinderRadius, true, false, 0.3);
              }
            }
            ...
            if(schema.inpos.x!==0 || schema.inpos.y!==0 || schema.inpos.z!==0){
                this.modelGroup.position.x = schema.inpos.x;
                this.modelGroup.position.y = schema.inpos.y;
                this.modelGroup.position.z = schema.inpos.z;
            }else{
              ...
              console.log(this.modelGroup.position.x);
    ...
    let dispose = function (parent,child){
    ...
    AFRAME.registerComponent('glmol', {
      ...
      update: function (oldData) {
        ...
        if  (oldData.molId !== this.data.molId) {
            this.glmol= new __WEBPACK_IMPORTED_MODULE_0_Lib_GLmol__["a" /* GLmol */](this.data);
            // Set mesh on entity.
            // alert("nnnnnnnn"+oldData.molId+"==!="+this.data.molId);
            ...
            // alert("wwww"+oldData.molId+"===="+this.data.molId);
            let arr = this.glmol.modelGroup.children.filter(x=>x);
            arr.forEach(a=>{dispose(this.glmol.modelGroup,a);});
            if ( this.data.mainchain === 'spacefill' || this.data.mainchain === 'ballAndStick') {
                //16;
                this.glmol.sphereQuality = 12;
                //16;
                this.glmol.cylinderQuality = 6;
            } else {
                this.glmol.sphereQuality = 16;
                this.glmol.cylinderQuality = 16;
            }
       ...
       */
      remove: function () { 
        // modelGroup = this.glmol.getModelGroup();
        let arr = this.glmol.modelGroup.children.filter(x=>x);
        arr.forEach(a=>{dispose(this.glmol.modelGroup,a);});
        this.el.removeObject3D('mesh', this.glmol.modelGroup);
        this.el.removeEventListener("loaded", this.emitGestureEvent);
      },
